...
def _runs(*specs):
    ...
    return [
        {"index": i, "edit_distance": d, "ts": None, "playground": pg}
...
def test_rover_rescue_fires_step_by_step_at_three():
    ...
    out = detect_run_triggers_by_playground(
        _runs(
            (None, "RoverRescue"),
            (2, "RoverRescue"),
            (2, "RoverRescue"),
...
def test_jump_back_to_a_playground_does_not_resume_the_old_count():
    ...
    out = detect_run_triggers_by_playground(
        _runs(
            (None, "RoverRescue"),
            (2, "RoverRescue"),  # visit 1: count 1
            (None, "CastleCrasherPlus"),
            (2, "CastleCrasherPlus"),
            (None, "RoverRescue"),
            (2, "RoverRescue"),  # visit 2: fresh count 1
        )
    )
    assert not any(t == "iterative" for t, _, _ in out)
...
def test_missing_playground_key_defaults_to_a_single_stretch():
    ...
    runs = [
        {"index": 0, "edit_distance": None, "ts": None},
