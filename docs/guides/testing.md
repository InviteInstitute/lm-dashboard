...
layer (including the datetime contract and the zip export), the ingestion poller, the
per-student workers, the APTED edit-distance computation, the five trigger rules, the
episode segmenter, and every API endpoint through a test client.
