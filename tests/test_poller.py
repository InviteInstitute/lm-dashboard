...
def test_backfill_respects_since_cutoff():
    ...
    assert inserted == 1
    assert [r["source_event_id"] for r in db._query("SELECT source_event_id FROM vex_log")] == [3]
...
def test_drain_respects_since_cutoff():
    ...
    assert new == 1
    assert [r["source_event_id"] for r in db._query("SELECT source_event_id FROM vex_log")] == [2]
