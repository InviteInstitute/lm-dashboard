...
from app import db
from app.runs.humanize import humanize_workspace, humanize_text
...
def test_simple_block_is_just_its_name():
    assert humanize_workspace(_wrap('<block type="pg_drivetrain_stop_driving"/>')) == ["stop driving"]


def test_keeps_the_value_number_and_hides_mutator_noise():
    xml = _wrap('<block type="pg_drivetrain_drive_for">'
...
def test_if_else_labels_the_else_branch():
    xml = _wrap('<block type="pg_control_if_then_else">'
...
def test_nested_reporters_recurse_to_any_depth():
    ...
    cond = ('<block type="pg_operator_not"><value name="OPERAND">'
            ...
            '</block></value></block>')
    xml = _wrap(f'<block type="pg_control_if_then"><value name="CONDITION">{cond}</value>'
...
def test_range_operator_keeps_all_three_numbers():
    xml = _wrap('<block type="pg_control_if_then"><value name="CONDITION">'
                '<block type="pg_operator_range"><field name="COMPARISON1">&lt;</field><field name="COMPARISON2">&lt;</field>'
                + _num("NUM1", 0) + _num("NUM2", 50) + _num("NUM3", 90) + '</block></value>'
                '<statement name="SUBSTACK"><block type="pg_drivetrain_stop_driving"/></statement></block>')
    top = humanize_workspace(xml)[0]
...
def test_detail_endpoint_exposes_readable_program(client, seed_state):
    ...
    xml = ('<xml><block type="pg_drivetrain_drive_for"><field name="DIRECTION">fwd</field>'
           '<field name="UNITS">mm</field>' + _num("AMOUNT", 200) + '</block></xml>')
    db.insert_message_and_log({
        "studentID": "cobra3", "classCode": "C1", "eventType": "blockMoved",
        "project": json.dumps({"workspace": xml}), "raw_message": "{}",
        "source_event_id": 1, "event_time": db.now()})
